...
- Enemies must not overlap the sides of the screen at the time of spwan.
...
- When (large) enemies collide with a bullet or player, they are destroyed,
  ...
  outward at angles at a fixed interval equal to (360 / vertices).
  For example, if the original enemy had 6 sides, the smaller enemies will
...
- Any special effects which do not alter game play can be added for upt to 
...
- You must be able to change the enemy spawn itnerval through the GUI
...
- You may develop the GUI in any way that contains this functionality, but 
  it must be easily usable and clearly presented to get full marks.
...
Window W H FL FS
- This lien declares that the SFML Window must be constructed with width W
...
7. Implement collisions in sCollisison and entity.destroy() if it's dead
...
9. Implement the GUI functionality. You may however want to implement the GUI
   functionality earlier to help you debug your program, it's upt to you.
